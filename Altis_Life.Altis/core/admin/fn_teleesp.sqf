...
if (isnil "mapm") then
{
	...
	["Adding map markers..."] call life_fnc_exp_hint;
...
else
{
	...
	sleep 0.5;
	["ESP and Markers removed..."] call life_fnc_exp_hint;
...
drawic =
{
	if (!isnil "mapm") then
	{
...

		{
			if (!isnull _x and sveh) then
			{
				...
				_color = switch(side _x) do{
					case (west) : {[0.0980392, 0.0980392, 0.439216, 1]};
					case (east) : {[0.5,0,0,1]};
...

		{
			...
			{
				if (_x == player) then {_color = [0, 0, 0, 1];} else {
					_color = switch(side _x) do{
						case (west) : {[0.0980392, 0.0980392, 0.439216, 1]};
						case (east) : {[0.5,0,0,1]};
						case (independent) : {[0,0.5,0,1]};
	...
	if (!isnil "mapm") then
	{
			...
			[] spawn
				{
					...
					{
						if(isNil "LIFE_ESPTOOL") then
						{
						LIFE_ESPTOOL = ["lifeesp","onEachFrame","playerESP"] call BIS_fnc_addStackedEventHandler;
						lhm_ID_PlayerTags = ["life_PlayerTags","onEachFrame","life_fnc_playerTags"] call BIS_fnc_addStackedEventHandler;
						};
					 sleep 1;
					};
				// Delete ESP Markers
				["lifeesp", "onEachFrame"] call BIS_fnc_removeStackedEventHandler;
				LIFE_ESPTOOL = nil;
				true
				};
		};
		...
		// Show Myself on MAP
		{
			if (!(_x in pList) and (vehicle _x == _x) and (getPlayerUID _x != "") and (visibleMap)) then
			{
				private ["_pos", "_mkr"];
				_pos = getPos _x;
				_isAdmin = _x getVariable ["isAdmin",0];

...
switch (_code) do {
    ...
    case _mapKey:
    {
        [] spawn life_fnc_mapMarkerplayer;
    };
    ...
    case 37: //K
    {
        if(_shift && vehicle player != player && vehicle player isKindOf "Landvehicle" && ((driver vehicle player) == player) && playerside in [west,east,independent]) then {
                if( !(vehicle player getvariable["W_light",false])) then {
                    [vehicle player,"W_light"] call life_fnc_light_switch;

                }else {
                    vehicle player setvariable["W_light",nil,true];
                    titletext["Warnblinker AUS","PLAIN DOWN"];
                };
            };
